    ...
    <div className="overflow-hidden rounded-xl border border-slate-200 glass">
      <div className="border-b border-slate-200 px-5 py-4">
        <div className="flex items-center gap-2.5">
          ...
          <h3 className="text-base font-semibold text-slate-900">{t("detection.title")}</h3>
        </div>
      </div>
      ...
      <div className="px-5 py-4">
        {isLoading && (
          <div className="flex flex-col items-center gap-3 py-8">
            <Spinner size="md" />
            <p className="text-sm text-slate-600">{t("detection.scanning")}</p>
          </div>
        )}
        ...
        {!isLoading && detection && !detection.has_visible_watermark && !detection.has_metadata && (
          <div className="flex flex-col items-center gap-3 py-6 text-center">
            ...
            <div>
              <p className="font-medium text-emerald-600">{t("detection.none")}</p>
              <p className="mt-1 text-sm text-slate-600">{t("detection.clean")}</p>
            </div>
          </div>
        )}
        ...
        {!isLoading && detection && (detection.has_visible_watermark || detection.has_metadata) && (
          <div>
            <div className="flex items-center gap-2.5 mb-4">
              ...
              <p className="font-medium text-slate-800">{t("detection.found")}</p>
            </div>
            ...
            <div className="space-y-2">
              {detection.has_visible_watermark && (
                <div className="flex items-center justify-between rounded-lg bg-amber-500/5 border border-amber-500/10 px-4 py-3">
                  <div className="flex items-center gap-2.5">
                    <span className="rounded-full bg-amber-500/15 px-2.5 py-0.5 text-xs font-medium text-amber-600">{t("detection.visible")}</span>
                    <span className="text-sm text-slate-700">{detection.watermark_type || "—"}</span>
                  </div>
              ...
              {detection.has_metadata && (
                <div className="flex items-center justify-between rounded-lg bg-blue-500/5 border border-blue-500/10 px-4 py-3">
                  <div className="flex items-center gap-2.5">
                    <span className="rounded-full bg-blue-500/15 px-2.5 py-0.5 text-xs font-medium text-blue-600">{t("detection.metadata")}</span>
            ...
            {detection.metadata_details && detection.metadata_details.length > 3 && (
              <div className="mt-3">
                <button onClick={() => setShowDetails(!showDetails)} className="flex items-center gap-1.5 text-xs font-medium text-brand-600 transition-colors hover:text-brand-700">
                  {t("detection.details")}
